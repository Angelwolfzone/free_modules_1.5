...
abstract class Export
{
     ...
     * @return string
     */
    static public function getProductLink($product, $id_shop = null, $context = false, $id_lang=false) {
	if ( ! $context) {
	    $context = Context::getContext();
	}
        ...
        if ($categories && sizeof($categories))
        {
            foreach ($categories as $category)
            {
                ...
                $prepared[$category['id_product']][$category['id_category']] =
                    self::$_existingCategories[$category['id_category']];
            }
        }
        return $prepared;
    }
     ...
     * @return float
     */
    static public function getShippingTax($carrier)
    {
        ...
        FROM `' . _DB_PREFIX_ . 'carrier` c
            ...
            ON (c.`id_tax_rules_group` = tr.`id_tax_rules_group`)
